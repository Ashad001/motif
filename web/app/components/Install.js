...
function highlight(code, comment) {
    return (<>
      ...
      {code.split("\n").map((line, i) => {
            ...
            if (keyMatch) {
                ...
                parts.push(<span key="indent">{indent}</span>, <span key="key" style={{ color: "rgba(240,240,240,0.7)" }}>
              {key}
            </span>, <span key="rest" style={{ color: "rgba(240,240,240,0.35)" }}>
              {rest.replace(/"[^"]*"$/, "")}
            </span>);
                if (strMatch) {
                    parts.push(<span key="val" style={{ color: "#86EFAC" }}>
                {strMatch[1]}
              </span>);
                    const afterVal = rest.replace(/.*"[^"]*"/, "");
                    if (afterVal)
                        parts.push(<span key="after" style={{ color: "rgba(240,240,240,0.35)" }}>
                  {afterVal}
                </span>);
                }
            }
            else {
                parts.push(<span key="plain" style={{ color: "rgba(240,240,240,0.35)" }}>
              {line}
            </span>);
            }
...
function Install() {
    const [tab, setTab] = (0, react_1.useState)("cursor");
    ...
    return (<div id="install" style={{ padding: "96px 6%", borderTop: "1px solid var(--border)" }}>
      {/* Header */}
      ...
      <div className="fade-up" style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          {["cursor", "claude"].map((t) => (<button key={t} onClick={() => setTab(t)} style={{
                ...mono,
                fontSize: "0.68rem",
                letterSpacing: "0.1em",
                textTransform: "uppercase",
                ...
                color: tab === t ? "var(--fg)" : "rgba(240,240,240,0.35)",
                cursor: "pointer",
                marginBottom: -1,
        ...
        <div style={{
          ...
          <button onClick={copy} style={{
            ...
            "rgba(240,240,240,0.12)")} onMouseLeave={(e) => (e.currentTarget.style.background =
